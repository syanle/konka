...
public class Matrix4f {
    ...
    public void loadMultiply(Matrix4f lhs, Matrix4f rhs) {
        for (int i = 0; i < 4; i++) {
            float ri0 = FlyingIcon.ANGULAR_VMIN;
            float ri1 = FlyingIcon.ANGULAR_VMIN;
            float ri2 = FlyingIcon.ANGULAR_VMIN;
            float ri3 = FlyingIcon.ANGULAR_VMIN;
            for (int j = 0; j < 4; j++) {
    ...
    public void loadOrthoWindow(int w, int h) {
        loadOrtho(FlyingIcon.ANGULAR_VMIN, (float) w, (float) h, FlyingIcon.ANGULAR_VMIN, -1.0f, 1.0f);
    }
    ...
    public void loadProjectionNormalized(int w, int h) {
        ...
        m2.loadRotate(180.0f, FlyingIcon.ANGULAR_VMIN, 1.0f, FlyingIcon.ANGULAR_VMIN);
        m1.loadMultiply(m1, m2);
        m2.loadScale(-2.0f, 2.0f, 1.0f);
        m1.loadMultiply(m1, m2);
        m2.loadTranslate(FlyingIcon.ANGULAR_VMIN, FlyingIcon.ANGULAR_VMIN, 2.0f);
        m1.loadMultiply(m1, m2);
        load(m1);
    }
